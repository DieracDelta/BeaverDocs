...
function testVim(name, run, opts, expectedFail) {
  ...
  return test('vim_' + name, function() {
    ...
    function doKeysFn(cm) {
      return function(args) {
        ...
        for (var i = 0; i < params.length; i++) {
          var result = CodeMirror.Vim.handleKey(cm, params[i]);
          if (!result && cm.state.vim.insertMode) {
            cm.replaceSelections(fillArray(params[i], cm.listSelections().length));
          }
        }
      }
    }
    function doInsertModeKeysFn(cm) {
      return function(args) {
        if (args instanceof Array) { params = args; }
        ...
        for (var i = 0; i < params.length; i++) {
          ...
          if (handled == "handled" && cm.state.vim.insertMode && params[i] != 'Esc') {
